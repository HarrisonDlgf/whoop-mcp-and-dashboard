...
function safe<Args extends unknown[]>(
  ...
  return async (...args: Args) => {
    try {
      ...
      return {
        ...json({ error: "Could not read from the WHOOP database." }),
        isError: true,
      };
    }
  };
...
function buildServer(): McpServer {
  ...
  server.registerTool(
    ...
    safe(async () => {
      ...
      if (!recovery) return json({ error: "No recovery data synced yet." });
  ...
  server.registerTool(
    "get_recovery_trends",
    {
      ...
      inputSchema: { days: z.number().int().min(1).max(180).default(14) },
    },
    safe(async ({ days }) => {
  ...
  server.registerTool(
    "get_sleeps",
    {
      ...
      inputSchema: { days: z.number().int().min(1).max(180).default(7) },
    },
    safe(async ({ days }) => {
